    ...
    <main className="min-h-screen bg-transparent text-neutral-900">
      <div className="max-w-md mx-auto px-5 pt-8 pb-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="text-sm text-[#B36B1E] font-semibold">03 早点</div>
            <h1 className="text-3xl font-black tracking-tight">
              earlier <span className="text-[#F59E0B]">早餐时间</span>
            </h1>
      ...
      <div className="max-w-md mx-auto px-4 pb-24">
        <div className="flex gap-3">
          ...
          <section className="flex-1">
            {loading ? (
              <div className="mt-6 text-sm text-neutral-500">正在加载菜品...</div>
            ) : (
          ...
          <div className="absolute left-0 right-0 top-1/3 mx-auto max-w-sm rounded-3xl bg-white p-6 shadow-2xl border border-[#F3D9B5]">
            ...
            <div className="mt-4 grid grid-cols-5 gap-2">
              {tableOptions.map((code) => (
                <button
                  key={code}
                  className="h-10 rounded-2xl border border-[#F3D9B5] bg-white font-black"
                  onClick={() => {
                    setTableCode(code);
